...
my ($mnt, @args) = @ARGV;
my $h = eval { Data::Heap::Shared->new("$mnt/x.shm", @args) };
print $h ? "created\n" : "refused: $@";
P
close $fh;
...
($sig, $out) = on_small_tmpfs('', 1_000_000);
...
($sig, $out) = on_small_tmpfs('DATA_HEAP_SHARED_SPARSE=1', 1_000_000);
is $sig, SIGBUS, 'DATA_HEAP_SHARED_SPARSE=1 skips the reservation';
